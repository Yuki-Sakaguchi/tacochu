...
import { createInput, toggleInput, updateInput } from "@/server/types/todo";

export const todoRouter = createTRPCRouter({
  all: protectedProcedure.query(async ({ ctx }) => {
    const todos = await ctx.prisma.todo.findMany({
      where: {
        userId: ctx.session.user.id, // アクセスしているユーザーのIDで絞り込む
      },
      orderBy: {
        createdAt: "desc",
      },
  ...
  create: protectedProcedure.input(createInput).mutation(({ ctx, input }) => {
    return ctx.prisma.todo.create({
      data: {
        ...
        user: {
          connect: {
            id: ctx.session.user.id, // ユーザーをリレーションで紐づける
          },
        },
      },
    });
